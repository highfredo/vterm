...
import { LocalProfileParams, ShellDimensions, ShellRequest, SshProfileParams } from 'src/types'
import { LocalConnection } from 'app/src-electron/shell/LocalShellConnector'
import { SshShellConnection } from 'app/src-electron/shell/SshShellConnector'
...
ipcMain.handle('shell:new', async (event: IpcMainInvokeEvent, shellRequest: ShellRequest) => {
  ...
  const connection = profile.type === 'local'  ?
