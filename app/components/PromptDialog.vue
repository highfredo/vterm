  ...
  <q-dialog ref="dialogRef" seamless @hide="onDialogHide">
    <q-card class="prompt-card">
      ...
      <form @submit.prevent="onDialogOK(responses)">
        <q-card-section>
          <div v-for="(prompt, idx) in prompts" :key="idx">
            <q-input v-if="prompt.echo" v-model="responses[idx]" filled :label="prompt.prompt" />
            <v-input-password v-else v-model="responses[idx]" filled :label="prompt.prompt" />
          </div>
        </q-card-section>
