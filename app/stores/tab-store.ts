...
export class Tab {
  ...
  on(name: string, listener: (...params: unknown[]) => void) {
...
export const useTabStore = defineStore('tabs', {
  ...
  actions: {
    async openSingle(tabRequest: TabRequest) {
      const tab = this.tabs.find((c) => c.component === tabRequest.component)
      if (tab) {
        await this.gotoTab(tab.id)
      } else {
        await this.open(tabRequest)
      }
    },
