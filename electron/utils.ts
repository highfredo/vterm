...
export const readFileParam = async (key: string): Promise<string> => {
  if (key.startsWith('file://')) {
    return await readFile(key.replace('file://', ''), { encoding: 'utf8' })
  }

  return key
}

export const eventToPromise = <T extends EventEmitter>(
  emitter: T,
  resolveEvt = 'ready',
  rejectEvt = 'error'
): Promise<T> => {
